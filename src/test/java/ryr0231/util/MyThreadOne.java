...
public class MyThreadOne extends Thread {
	...
	public MyThreadOne(ArrayList<String> list, int icolumn, MyCountDownOne c,
			int iclass,int top_k) {
		this.list = list;
	...
	public void run() {
		...
		try {
			if (iclass == 0) { // iclass=0 标志是寻找表头， 1标志寻找主键
				...
				// 得到一个类的实例集,返回了7个
				arrayList = probaseAPI.GetOneClassSetByInstanceSet(list, top_k,
						false);
	...
	protected ArrayList<NameScorePair> secondintergrate(ArrayList<String> ilist)
		...
		ProbaseAPI probaseAPI = new ProbaseAPI();
		ArrayList<NameScorePair> arrayList = null;
		arrayList = probaseAPI
				.GetOneClassSetByInstanceSet(iArrayList, 5, false);
		return arrayList;
	}

	// 调用了outprint方法，打印出7个[maxconcept,maxscore]
	protected synchronized void outprint(ArrayList<NameScorePair> arrayList) {
